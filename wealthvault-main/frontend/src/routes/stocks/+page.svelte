<script lang="ts">
  ...
  import {
    stocks, stocksLoading, loadStocks, addStock,
    updateStock, sellStock, deleteStock
  } from '$lib/stores/stocks';
  import { familyMembers, loadFamily } from '$lib/stores/family';
    ...
    <div class="flex gap-2 overflow-x-auto pb-1">
      <button
        ...
        class="px-4 py-2 rounded-lg text-sm font-medium transition whitespace-nowrap
          {activeTab === 'my' ? 'bg-emerald-500 text-black' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}"
      >
        My Stocks
      </button>
      {#each $familyMembers as member}
        {#if member.uid !== $user?.uid}
          <button
            ...
            class="px-4 py-2 rounded-lg text-sm font-medium transition whitespace-nowrap
              {activeTab === member.uid ? 'bg-emerald-500 text-black' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}"
          >
            {member.name}
          </button>
        {/if}
            ...
            {#each $stocks as stock}
              <tr class="border-b border-gray-800 hover:bg-gray-800/50 transition">
                ...
                <td class="px-3 py-3 text-right">
                  <button
                    on:click={() => { selectedStock = stock; newRate = stock.todaysRate; showRateModal = true; }}
                ...
                <td class="px-3 py-3 text-right {plColor(stock.plPercent)}">{stock.plPercent?.toFixed(2)}%</td>
                ...
                <td class="px-3 py-3 text-right {plColor(stock.diffPL)}">{formatINR(stock.diffPL)}</td>
                ...
                <td class="px-3 py-3 text-center">
                  <button
                    ...
                    class="w-5 h-5 rounded border-2 transition flex items-center justify-center mx-auto
                      {stock.releaseStatus ? 'bg-emerald-500 border-emerald-500' : 'border-gray-600 hover:border-emerald-500'}"
                  >
                ...
                <td class="px-3 py-3 text-center">
                  <div class="flex items-center gap-2 justify-center">
                    <button
                      on:click={() => { selectedStock = stock; showSellModal = true; }}
    ...
    <div class="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md">
      ...
      <div class="space-y-3">
        ...
        <div>
          ...
          <input type="text" bind:value={form.shareName} placeholder="Reliance Industries"
            class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-emerald-500 outline-none"/>
        </div>
        <div>
          <label class="text-gray-400 text-xs block mb-1">Short Name (NSE Symbol)</label>
          <input type="text" bind:value={form.shortName} placeholder="RELIANCE"
            class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-emerald-500 outline-none"/>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
      ...
      <div class="flex gap-3 mt-5">
        ...
        <button on:click={handleAddStock}
...
{#if showSellModal && selectedStock}
  <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
    <div class="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm">
      ...
      <input type="number" bind:value={sellQty} max={selectedStock.qtyHeld}
      ...
      <div class="flex gap-3">
        <button on:click={() => showSellModal = false}
          class="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-2 rounded-xl transition">
          Cancel
        </button>
        <button on:click={handleSell}
  ...
  <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
    <div class="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm">
      ...
      <input type="number" bind:value={newRate}
        class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:border-emerald-500 outline-none mb-5"/>
      <div class="flex gap-3">
        <button on:click={() => showRateModal = false}
          class="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-2 rounded-xl transition">
          Cancel
        </button>
        <button on:click={handleRateUpdate}
