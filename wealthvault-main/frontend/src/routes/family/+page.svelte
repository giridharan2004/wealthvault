	...
	{:else}
		<div class="bg-gray-900 border border-gray-800 rounded-2xl p-10 w-full max-w-md">
			...
			<div class="flex bg-gray-800 rounded-xl p-1 mb-8">
				<button
					onclick={() => (mode = 'create')}
					class="flex-1 py-2 rounded-lg text-sm font-medium transition {mode === 'create'
						? 'bg-emerald-500 text-black'
						: 'text-gray-400 hover:text-white'}">Create Family</button
				>
				<button
					onclick={() => (mode = 'join')}
					class="flex-1 py-2 rounded-lg text-sm font-medium transition {mode === 'join'
						? 'bg-emerald-500 text-black'
						: 'text-gray-400 hover:text-white'}">Join Family</button
				>
			</div>
